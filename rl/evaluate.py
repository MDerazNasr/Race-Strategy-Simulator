...
def evaluate_all(n_episodes: int = 20, fixed_start: bool = False) -> List[PolicySummary]:
    ...
    try:
        ...
        env_multi_d41 = None
    # Intermediate multi-agent env (13D obs, 2D actions) — for d43 policy.
    # Opponent at 25.0 m/s — 2 m/s below ego top speed (sweet spot between
    # D39's trivial 22 m/s and D41's impossible 27 m/s).
    try:
        from env.f1_multi_env import F1MultiAgentEnv as _F1MA43
    ...
    try:
        from env.f1_multi_env import F1MultiAgentEnv as _F1MA44
        env_multi_d44 = _F1MA44(opp_max_speed=25.0, position_bonus=2.0)
    except ImportError:
        env_multi_d44 = None
    ...
    configs = [
        ...
        {
            "name":          "PPO Pit v4 Continued (d22)",
            "color":         "#00BCD4",
            # d22: continued training from ppo_pit_v4 for 2M more steps.
            # Goal: sharpen pit timing, reduce stochastic over-pitting,
            # improve post-pit driving stability.
            "fn":            lambda: make_ppo_policy(
                str(project_root / "rl" / "ppo_pit_v4_cont.zip"), device,
                obs_dim=12,
            ),
            "optional":      True,
        ...
        {
            "name":  "PPO Multi-Agent D43 (d43)",
            "color": "#AB47BC",
            # D43: Intermediate opponent — sweet spot between D39 (trivial) and D41 (too hard).
            # D39 (opp=22): col[11] weight=0, 100% laps — no strategy, pure speed.
            # D41 (opp=27): col[11] weight>0, 0% laps — strategy activated, but eval broken.
            # D43 (opp=25): hypothesis — col[11] weight>0 AND reliable lap completion.
            # Warm-start from D41 which already has non-zero positional weights.
            "fn":            lambda: make_ppo_policy(
                str(project_root / "rl" / "ppo_multi_agent_d43.zip"), device,
                obs_dim=13,
            ),
            "optional":      True,
            "optional_file": "ppo_multi_agent_d43.zip",
            "env_key":       "multi_d43",
        },
        {
            "name":  "PPO Multi-Agent D44 (d44)",
            "color": "#7B1FA2",
            # D44: Stronger position bonus — breaks the follow equilibrium.
            # D43 (pos_bonus=0.5) converged to "follow at opponent speed" → 0 laps.
            # Root cause: deterministic ego speed (24.36) < opponent (25) = never ahead.
            # Fix: position_bonus 0.5 → 2.0 makes being ahead 4x more valuable.
            # Warm-start from D43 which already has non-zero track_gap weights.
            "fn":            lambda: make_ppo_policy(
                str(project_root / "rl" / "ppo_multi_agent_d44.zip"), device,
                obs_dim=13,
            ),
            "optional":      True,
    ...
    for cfg in configs:
        ...
        elif cfg.get("env_key") == "multi_d43":
            if env_multi_d43 is None:
                print(f"  Skipping: {cfg['name']} (env.f1_multi_env not available)")
                continue
            eval_env = env_multi_d43
        elif cfg.get("env_key") == "multi_d44":
            if env_multi_d44 is None:
                print(f"  Skipping: {cfg['name']} (env.f1_multi_env not available)")
                continue
            eval_env = env_multi_d44
